import type { BrandKitDecorativeAsset } from "@onbrand/core/brand-guide/application-service";
    ...
    {decorativeAssets.length === 0 ? (
      ...
      <div className="columns-2 gap-2.5 md:columns-4 lg:columns-5">
        {decorativeAssets.map((asset) => (
          <AssetShowcaseCard key={asset.id} asset={asset} brandGuideId={brandGuideId} />
        ))}
      </div>
    ) : (
      <div className="grid gap-1.5 md:grid-cols-2">
        {decorativeAssets.map((asset) => (
          <AssetShowcaseListItem key={asset.id} asset={asset} brandGuideId={brandGuideId} />
        ))}
      </div>
    )}
  </section>
