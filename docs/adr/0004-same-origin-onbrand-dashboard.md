...
users authenticate through the configured OAuth provider using a static dashboard client and receive
an Onbrand HttpOnly session cookie. MCP clients continue to authenticate with bearer access tokens
on `/mcp`.
...
boundaries intact: the OAuth provider remains the identity system; Onbrand remains the owner of
Brand Guides, Brand Kits, Presentation Kits, assets, MCP tools, and dashboard APIs.

A future split to `app.onbrand.example.com` remains possible if the dashboard outgrows same-origin
